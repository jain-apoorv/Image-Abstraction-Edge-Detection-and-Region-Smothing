...
def visualize_etf(etf, scale=10, color=(255, 255, 255), thickness=1):
    ...
    for y in range(0, height, scale):
        for x in range(0, width, scale):
            ...
            x2 = int(x + dx * scale)
            y2 = int(y + dy * scale)
            cv2.arrowedLine(flow_image, (x, y), (x2, y2), color, thickness)
...
def main(input_path, output_path, flags):
	...
	if os.path.isfile('tmp\\' + name + '_etf.pickle'):
		...
		time1 = time.time()
		etf = ETF(gray_image, iterations = 1, mu = 4, type = 0)   # mu = 5 initially, 
		time2 = time.time()
	...
	for j in range(len(edges)):
		...
		# var = images
		# var = np.stack([edges[j], edges[j], edges[j]], axis=2)
		# etf_magnitude = np.linalg.norm(etf, axis=2)
		# etf_norm = (etf_magnitude / np.max(etf_magnitude) * 255).astype(np.uint8)
		# var = np.stack([etf_norm, etf_norm, etf_norm], axis=2)


		# var  = visualize_etf(etf)
